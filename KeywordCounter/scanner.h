//       $Id: scanner.h 47996 2023-12-22 12:20:50Z p20068 $
//      $URL: https://svn01.fh-hagenberg.at/bin/cepheiden/pfc/trunk/pfc/inc/pfc/scanner.h $
// $Revision: 47996 $
//     $Date: 2023-12-22 13:20:50 +0100 (Fr., 22 Dez 2023) $
//   $Author: p20068 $
...
#define PFC_SCANNER_VERSION "2.6.4"
...
class scanner final {
   public:
     ...
     ~scanner () = default;
      ...
      void report_eol (bool const set = true) {
         m_report_eol = set;
      }
   ...
   private:
      ...
      void read_next_symbol () {
         ...
         while (!scanned) {
            if (is_eof_chr ())
            ...
            else if (m_report_eol && is_eol_chr ()) {
               m_current_symbol = scn::details::sk_eol;
               scanned          = true;

               read_next_chr (scn::details::sk_eol.chr);
               ...
               if ((scanned = m_current_symbol.is_terminal_symbol ()) == false) {
                  scn::exception const x {"Unknown character '"s + m_current_chr + "' encountered."}; read_next_chr (); throw x;
               }
